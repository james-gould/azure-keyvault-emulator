> [!CAUTION]
> This is currently out of date and requires rewriting. The `Dockerfile` is up to date and used to publish to [the Docker registry](https://hub.docker.com/r/jamesgoulddev/azure-keyvault-emulator), the `makefile` and associated `.sh` scripts have been removed. These will be available in the future.
