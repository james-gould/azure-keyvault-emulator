...
namespace AzureKeyVaultEmulator.Shared.Utilities
{
    public static class HttpRequestUtils
    {
        ...
        /// </summary>
        public static string GetNextLink(this HttpContext context, string skipToken, int max = 25)
        {
            var builder = new Uri($"{context.Request.Scheme}://{context.Request.Path}{context.Request.Path}");
