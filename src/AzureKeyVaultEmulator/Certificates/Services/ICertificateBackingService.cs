...
public interface ICertificateBackingService
{
    ...
    Task<(KeyBundle backingKey, SecretBundle backingSecret)> GetBackingComponentsAsync(string certName, X509Certificate2Collection collection, string? certificatesPassword, CertificatePolicy? policy = null, X509ContentType contentType = X509ContentType.Pfx);
    Task<(DeletedKeyBundle deletedBackingKey, DeletedSecretBundle deletedBackingSecret)> DeleteBackingComponentsAsync(string certName);
