...
> [!CAUTION]
> Using `.WaitForCompletion(keyVault)` will currently hang the `AppHost`, use `.WaitFor(keyVault)`. 
...
> [!NOTE]
> There's a [pending PR](https://github.com/dotnet/aspire/pull/8408) to add support for the `KeyClient` and `CertificateClient` into the new `Aspire.Security.Azure.Client` package. Support for these 2 clients is expected in `.NET Aspire 9.3`.
